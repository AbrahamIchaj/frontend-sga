    ...
    <div class="">
      ...
      <div class="flex justify-between items-center mb-6">
        ...
        <button 
          ...
          class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-6 rounded-lg transition duration-200 flex items-center space-x-2 border border-blue-500">
          <span>Nuevo Usuario</span>
        </button>
      </div>

      <!-- Filtros -->
      <div class="bg-[#1e293b] rounded-lg shadow-sm p-6 mb-6 text-gray-100">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          ...
          <div>
            ...
            <select 
              ...
              <option *ngFor="let rol of rolesDisponibles" [value]="rol.idRoles">{{rol.nombreRol}}</option>
            </select>
          </div>
        </div>
      </div>
      ...
      <div class="bg-[#1e293b] rounded-lg shadow overflow-hidden">
        ...
        <div *ngIf="!cargando" class="overflow-x-auto">
          <table class="w-full">
            ...
            <tbody class="bg-[#1e293b] divide-y divide-[#334155]">
              <tr *ngFor="let usuario of usuariosFiltrados" class="hover:bg-[#232e47]">
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="flex items-center">
                    <div class="flex-shrink-0 h-10 w-10">
                      <div class="h-10 w-10 rounded-full bg-gray-600 flex items-center justify-center">
                        <span class="text-sm font-medium text-gray-200">{{getIniciales(usuario.nombres, usuario.apellidos)}}</span>
                      </div>
                    </div>
                    <div class="ml-4">
                      <div class="text-sm font-medium text-gray-100">{{usuario.nombres}} {{usuario.apellidos}}</div>
                      <div class="text-sm text-gray-400">ID: {{usuario.idUsuario}}</div>
                    </div>
                  </div>
                </td>
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="flex flex-col space-y-1">
                    <span [class]="getEstadoClases(usuario.activo)" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium">
                      {{usuario.activo ? 'Activo' : 'Inactivo'}}
                    </span>
                    <span *ngIf="usuario.esTemporal" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-orange-100 text-orange-800">
                      <i class="fas fa-key mr-1"></i>
                      Contraseña temporal
                    </span>
                    <span *ngIf="usuario.debesCambiarPassword" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">
        ...
        <!-- Mensaje sin datos -->
        <div *ngIf="!cargando && usuariosFiltrados.length === 0" class="p-8 text-center">
          <div class="text-gray-400">
            <span class="text-4xl mb-4 block">👥</span>
            <p class="text-lg">No se encontraron usuarios</p>
            <p class="text-sm">Prueba ajustando los filtros o crea un nuevo usuario</p>
          </div>
        </div>

        <!-- Paginación -->
        <div *ngIf="!cargando && usuariosFiltrados.length > 0" class="px-6 py-3 border-t border-[#334155] bg-[#232e47]">
...
export class ListaUsuariosComponent implements OnInit {
  ...
  aplicarFiltros() {
    this.usuariosFiltrados = this.usuarios.filter(usuario => {
      const coincideBusqueda = !this.filtro.busqueda || 
        usuario.nombres.toLowerCase().includes(this.filtro.busqueda.toLowerCase()) ||
        usuario.apellidos.toLowerCase().includes(this.filtro.busqueda.toLowerCase()) ||
        usuario.correo.toLowerCase().includes(this.filtro.busqueda.toLowerCase());

      const coincideEstado = !this.filtro.estado || 
        usuario.activo.toString() === this.filtro.estado;

      const coincideRol = !this.filtro.rol || 
  ...
  eliminarUsuario(usuario: Usuario) {
    ...
    if (confirm(`¿Estás seguro de que deseas eliminar al usuario ${usuario.nombres} ${usuario.apellidos}?`)) {
